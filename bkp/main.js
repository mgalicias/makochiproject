...
function initScrollAnimations() {
    ...
    const animatedElements = document.querySelectorAll('.card, .team-member, .pilar-item, .area-card, .taller-card, .gallery-item, .caracteristica-item, .paso-item, .costo-card, .fecha-item, .faq-item');
...
function initHeroTitleAnimation() {
    ...
    words.forEach((word, wordIndex) => {
        ...
        word.split('').forEach((char, charIndex) => {
            ...
            span.style.cssText = `
                display: inline-block;
                animation: jumpLetter 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55) ${(wordIndex * word.length + charIndex) * 0.08}s both;
...
function createParticles(element) {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.cssText = `
            ...
            background: ${['var(--color-primary)', 'var(--color-secondary)', 'var(--color-accent)'][Math.floor(Math.random() * 3)]};
            border-radius: 50%;
            pointer-events: none;
            z-index: 1000;
        `;
...
function animateWordExplosion(wordElement) {
    ...
    letters.forEach((letter, index) => {
        ...
        letter.animate([
            ...
            { 
                transform: `${originalTransform} scale(2) rotate(${Math.random() * 360}deg) translate(${(Math.random() - 0.5) * 100}px, ${(Math.random() - 0.5) * 100}px)`,
                background: `linear-gradient(45deg, ${['var(--color-primary)', 'var(--color-accent)', 'var(--color-secondary)'][Math.floor(Math.random() * 3)]}, ${['var(--color-primary)', 'var(--color-accent)', 'var(--color-secondary)'][Math.floor(Math.random() * 3)]})`
            },
            { transform: originalTransform }
        ], {
            duration: 1000,
